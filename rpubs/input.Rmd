...
dat <- dat_raw %>%
  ...
  select(
    Make, Model, Device, "Connection" = `Wired / Wireless`, "WirelessConnection" = `Wireless Connection`, `Home Button`,
    "Latency" = `Average Measured Latency (in ms)`,
    "SD" = `Standard Deviation of Measured Latency (in ms)`,
    "Max" = `Max Measured Latency (in ms)`,
    "Min" = `Min Measured Latency (in ms)`,
    Category, `Face Buttons`, Row
  ) %>%
  filter(!is.na(Latency)) %>%
  arrange(Latency) %>%
  mutate(
    `Missed` = ifelse(Latency < 100 / 6, Latency / (100 / 6), 1),
    `On Time` = 1 - Missed,
    ID = row_number()
  ) %>%
...
```{r, echo=FALSE}
keep <- c("Device", "Latency", "Category", "Row")

shared_dat <- SharedData$new(dat)
bscols(widths = c(12),
  list(
    filter_select("Make", "Make", shared_dat, ~Make),
    filter_checkbox("Connection", "Connection", shared_dat, ~Connection, inline = TRUE),
...
fit <- lm(data=dat, SD ~ Latency + Min)
dat$Imputed <- is.na(dat$SD)
dat$SD[dat$Imputed] <- predict.lm(fit, dat[dat$Imputed,])

dat <- dat %>%
    mutate(
        `SD up` = Latency + qnorm(.975) * SD,
        `SD down` = Latency - qnorm(.975) * SD
    )
```


```{r, echo=FALSE, fig.height=10}
ggplot(dat, aes(x = Connection, y = Latency, color = Connection)) +
  geom_boxplot() +
...
```{r, echo=FALSE}
ggplot(dat) +
  geom_point(aes(x = 1:nrow(dat), y = Latency, color = Connection)) +
  scale_color_manual(values = c("Wired" = '#2708A0', "Wireless" = '#F9C80E')) +
  geom_hline(yintercept = 100 / 6, color = '#BA1B1D', linetype = "dashed") +
  xlab("Rank") +
  ylab("Latency (in ms)") +
...
```{r, echo=FALSE, fig.height=10}
dat %>%
  filter(Connection == "Wireless") %>%
  ggplot(aes(x = reorder(Device, desc(Device)), y = Latency)) +
  geom_errorbar(aes(ymin=`SD down`, ymax=`SD up`, color = factor(Imputed, levels = c(TRUE, FALSE)))) +
  geom_hline(yintercept = 100 / 6, color = '#BA1B1D', linetype = "dashed") +
  geom_hline(yintercept = 0, color = '#2708A0') +
  coord_flip() +
