...
```{r wired-errorbar, echo=FALSE, fig.height=20, fig.width=14, warning=FALSE}
wired_plot_dat <- dat %>%
...
# Calculate dynamic height based on number of devices (20px per device, min 400)
wired_height <- max(400, nrow(wired_plot_dat) * 22)

if (nrow(wired_plot_dat) > 0) {
  p_wired_range <- wired_plot_dat %>%
    ggplot(aes(x = reorder(DeviceShort, desc(`Latency (in ms)`)),
               y = `Latency (in ms)`,
    ...
    labs(title = "Wired Device Latency Ranges (95% CI)",
         ...
         x = "", y = "Latency (ms)", color = "SD Source") +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 7),
          plot.margin = margin(5, 10, 5, 5))

  ggplotly(p_wired_range, tooltip = "text", height = wired_height, width = 1400) %>%
    layout(hovermode = "closest",
           margin = list(l = 280, t = 50, b = 50))
}
```

## Wireless Device Latency Ranges

```{r wireless-errorbar, echo=FALSE, fig.height=14, fig.width=14, warning=FALSE}
...
wireless_height <- max(400, nrow(wireless_plot_dat) * 22)
...
if (nrow(wireless_plot_dat) > 0) {
  ...
  ggplotly(p_wireless_range, tooltip = "text", height = wireless_height, width = 1400) %>%
    layout(hovermode = "closest",
           margin = list(l = 280, t = 50, b = 50))
}
```
...
Top performing devices in each category, separated by wired and wireless connection.

```{r best-performers, echo=FALSE}
# Get best wired device per category
best_wired <- dat %>%
  filter(`Wired/Wireless` == "Wired", !is.na(Category)) %>%
  group_by(Category) %>%
  slice_min(`Latency (in ms)`, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  mutate(Connection = "Wired") %>%
  select(Category, Device = DeviceClean, `Latency (in ms)`, Connection)

# Get best wireless device per category
best_wireless <- dat %>%
...
best_performers <- bind_rows(best_wired, best_wireless) %>%
  arrange(Category, Connection) %>%
  mutate(`Latency (in ms)` = round(`Latency (in ms)`, 2))
...
datatable(best_performers,
          ...
          rownames = FALSE,
          caption = "Best performing device in each category (lowest latency)")
```
