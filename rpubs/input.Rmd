...
```{r wired-wireless-plotly, echo=FALSE, fig.height=8, warning=FALSE}
p_wired <- dat %>%
...
ggplotly(p_wired, tooltip = c("text", "y")) %>%
  layout(hovermode = "closest")
```

## Bluetooth vs. 2.4 GHz Wireless

```{r bt-vs-24ghz-plotly, echo=FALSE, fig.height=8, warning=FALSE}
wireless_dat <- dat %>%
...
if (nrow(wireless_dat) > 0) {
  p_wireless <- wireless_dat %>%
    ...
    xlab("Wireless Type") +
    ylab("Latency (in ms)") +
    labs(title = "Bluetooth vs. 2.4 GHz Wireless Latency") +
    theme_minimal() +
    theme(legend.position = "none")

  ggplotly(p_wireless, tooltip = c("text", "y")) %>%
    layout(hovermode = "closest")
}
```
...
```{r latency-rank-plotly, echo=FALSE, warning=FALSE}
dat_ranked <- dat %>%
  arrange(`Latency (in ms)`) %>%
...
ggplotly(p_rank, tooltip = c("text", "y", "x")) %>%
  layout(hovermode = "closest")
```

## Category Comparison

```{r category-violin-plotly, echo=FALSE, fig.height=10, warning=FALSE}
p_category <- dat %>%
...
ggplotly(p_category, tooltip = c("text", "y")) %>%
  layout(hovermode = "closest")
```
...
```{r raw-histogram-plotly, echo=FALSE, fig.height=8, warning=FALSE}
if (nrow(raw_data) > 0) {
  # Get top 20 devices by reading count for cleaner visualization
  ...
  p_hist <- raw_subset %>%
    ggplot(aes(x = delay, fill = Device)) +
...
```{r raw-timeseries-plotly, echo=FALSE, fig.height=10, warning=FALSE}
if (nrow(raw_data) > 0) {
  # Select a subset of interesting devices for pattern analysis
  pattern_devices <- raw_data %>%
    group_by(Device) %>%
    filter(n() >= 200) %>%
    slice_head(n = 200) %>%
    ungroup()

  if (nrow(pattern_devices) > 0) {
    # Limit to 8 devices for readability
    selected_devices <- pattern_devices %>%
      distinct(Device) %>%
      head(8) %>%
      pull(Device)

    pattern_subset <- pattern_devices %>%
      filter(Device %in% selected_devices)

    p_pattern <- pattern_subset %>%
      ggplot(aes(x = read, y = delay, color = Device)) +
      geom_line(alpha = 0.7) +
      geom_point(size = 0.5, alpha = 0.5) +
      facet_wrap(~Device, scales = "free_y", ncol = 2) +
      geom_hline(yintercept = 100 / 6, color = '#BA1B1D', linetype = "dashed") +
      labs(title = "Latency Time Series (First 200 Readings)",
           subtitle = "Look for periodic sawtooth patterns indicating polling intervals",
...
```{r consistency-plotly, echo=FALSE, warning=FALSE}
if (nrow(raw_stats) > 0) {
  p_consistency <- raw_stats %>%
    filter(!is.na(Consistency)) %>%
    ggplot(aes(x = RawMean, y = Consistency, text = Device)) +
  ...
  ggplotly(p_consistency, tooltip = c("text", "x", "y")) %>%
    layout(hovermode = "closest")
}
```
...
## Wired Device Latency Ranges

```{r wired-errorbar, echo=FALSE, fig.height=15, warning=FALSE}
wired_plot_dat <- dat %>%
  filter(`Wired/Wireless` == "Wired") %>%
  mutate(DeviceLabel = gsub("<[^>]+>", "", Device)) # Remove HTML for plotting

if (nrow(wired_plot_dat) > 0) {
  p_wired_range <- wired_plot_dat %>%
    ggplot(aes(x = reorder(DeviceLabel, desc(`Latency (in ms)`)),
               y = `Latency (in ms)`,
               text = paste0(DeviceLabel, "\nLatency: ", round(`Latency (in ms)`, 2), " ms"))) +
    geom_errorbar(aes(ymin = `SD down`, ymax = `SD up`,
                      color = factor(Imputed, levels = c(TRUE, FALSE))),
                  width = 0.3) +
    geom_point(size = 1) +
    geom_hline(yintercept = 100 / 6, color = '#BA1B1D', linetype = "dashed") +
    geom_hline(yintercept = 0, color = '#2708A0', alpha = 0.3) +
    coord_flip() +
    scale_color_manual(values = c("TRUE" = "#E63946", "FALSE" = "#457B9D"),
                       labels = c("TRUE" = "Imputed SD", "FALSE" = "Measured SD")) +
    labs(title = "Wired Device Latency Ranges (95% CI)",
         caption = "Note: Ranges assume normal distribution. Red bars indicate imputed standard deviation.",
         x = "Device", y = "Latency (ms)", color = "SD Source") +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 7))

  ggplotly(p_wired_range, tooltip = "text") %>%
    layout(hovermode = "closest")
}
```

## Wireless Device Latency Ranges

```{r wireless-errorbar, echo=FALSE, fig.height=10, warning=FALSE}
wireless_plot_dat <- dat %>%
  filter(`Wired/Wireless` == "Wireless") %>%
  mutate(DeviceLabel = gsub("<[^>]+>", "", Device))

if (nrow(wireless_plot_dat) > 0) {
  p_wireless_range <- wireless_plot_dat %>%
    ggplot(aes(x = reorder(DeviceLabel, desc(`Latency (in ms)`)),
               y = `Latency (in ms)`,
               text = paste0(DeviceLabel, "\nLatency: ", round(`Latency (in ms)`, 2), " ms"))) +
    geom_errorbar(aes(ymin = `SD down`, ymax = `SD up`,
    ...
    scale_color_manual(values = c("TRUE" = "#E63946", "FALSE" = "#457B9D"),
                       labels = c("TRUE" = "Imputed SD", "FALSE" = "Measured SD")) +
    labs(title = "Wireless Device Latency Ranges (95% CI)",
         caption = "Note: Ranges assume normal distribution. Red bars indicate imputed standard deviation.",
         x = "Device", y = "Latency (ms)", color = "SD Source") +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 8))

  ggplotly(p_wireless_range, tooltip = "text") %>%
    layout(hovermode = "closest")
}
```
