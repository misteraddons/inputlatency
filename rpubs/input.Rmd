...
```{r category-violin-plotly, echo=FALSE, fig.height=10, fig.width=14, warning=FALSE}
p_category <- dat %>%
...
if (nrow(raw_data) > 0) {
  # Get top 16 devices by reading count for cleaner visualization
  ...
  ggplotly(p_hist, height = 600, width = 1400) %>%
    layout(hovermode = "closest")
}
```
...
```{r wired-errorbar, echo=FALSE, fig.height=18, fig.width=14, warning=FALSE}
wired_plot_dat <- dat %>%
  filter(`Wired/Wireless` == "Wired") %>%
  mutate(
    DeviceLabel = gsub("<[^>]+>", "", Device),
    DeviceShort = truncate_name(DeviceLabel, 50)
  )
...
if (nrow(wired_plot_dat) > 0) {
  ...
  ggplotly(p_wired_range, tooltip = "text", height = 900, width = 1400) %>%
    layout(hovermode = "closest",
           margin = list(l = 250))
}
```
...
```{r wireless-errorbar, echo=FALSE, fig.height=12, fig.width=14, warning=FALSE}
...
if (nrow(wireless_plot_dat) > 0) {
  ...
  ggplotly(p_wireless_range, tooltip = "text", height = 600, width = 1400) %>%
    layout(hovermode = "closest",
           margin = list(l = 250))
}
```
